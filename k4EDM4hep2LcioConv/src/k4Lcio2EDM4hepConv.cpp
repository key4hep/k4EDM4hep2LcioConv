...
namespace LCIO2EDM4hepConv {
  edm4hep::TrackState convertTrackState(const EVENT::TrackState *trackState) {
    ...
    edmtrackState.covMatrix = {covMatrix[0],  covMatrix[1],  covMatrix[2],  covMatrix[3], covMatrix[4],  covMatrix[5],
                               covMatrix[6],  covMatrix[7],  covMatrix[8],  covMatrix[9], covMatrix[10], covMatrix[11],
                               covMatrix[12], covMatrix[13], covMatrix[15], 0,            0,             0,
                               0,             0,             0};
      ...
      for (auto d : daughters) {
        ...
        if (it != mcparticlesMap.end()) {
          ...
          std::cerr << "Cannot find corresponding EDM4hep MCParticle for an LCIO MCParticle, "
                       "while trying to resolve the daughters of MCParticles" << std::endl;
        }
      }
      for (auto p : parents) {
        if (p == nullptr) {
        ...
        if (it != mcparticlesMap.end()) {
          ...
          std::cerr << "Cannot find corresponding EDM4hep MCParticle for the LCIO MCParticle, "
                       "while trying to resolve the parents of MCParticles Collections" << std::endl;
        }
      }
    }
  }

  void resolveRelationsSimTrackerHit(TypeMapT<const lcio::SimTrackerHit *, edm4hep::MutableSimTrackerHit> &SimTrHitMap,
                                     TypeMapT<const lcio::MCParticle *, edm4hep::MutableMCParticle> &mcparticlesMap) {
    for (auto & [ lcio, edm ] : SimTrHitMap) {
      ...
      if (it != mcparticlesMap.end()) {
        ...
        std::cerr << "Cannot find corresponding EDM4hep MCParticle for the LCIO MCParticle, "
                     "while trying to resolve the SimTrackHit Relations" << std::endl;
      }
    }
  }
    ...
    for (auto & [ lcio, edm ] : recoparticlesMap) {
      ...
      if (const auto it = vertexMap.find(vertex); it != vertexMap.end()) {
        ...
        std::cerr << "Cannot find corresponding EDM4hep Vertex for a LCIO Vertex, "
                     "while trying to resolve the ReconstructedParticle Relations " << std::endl;
      }
      ...
      for (auto p : parents) {
        ...
        if (it != recoparticlesMap.end()) {
          ...
          std::cerr << "Cannot find corresponding EDM4hep RecoParticle for a LCIO RecoParticle, "
                       "while trying to resolve the ReconstructedParticles parents Relations" << std::endl;
        }
      }
    }
  }
  ...
  std::unique_ptr<podio::CollectionBase> fillSubset(EVENT::LCCollection *LCCollection,
                                                    const LcioEdmTypeMapping &typeMapping,
